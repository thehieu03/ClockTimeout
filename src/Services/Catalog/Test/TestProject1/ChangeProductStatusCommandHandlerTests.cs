...
[TestClass]
public sealed class ChangeProductStatusCommandHandlerTests
{
    private Mock<IDocumentSession> _mockSession = null!;
    private Mock<IMediator> _mockMediator = null!;
    private ChangeProductStatusCommandHandler _handler = null!;
    ...
    public void Setup()
    {
        ...
        _handler = new ChangeProductStatusCommandHandler(_mockSession.Object, _mockMediator.Object);
    }

    [TestMethod]
    public async Task Handle_WithValidCommand_ShouldChangeStatusAndReturnProductId()
    {
        // Arrange
        var productId = Guid.NewGuid();
        var actor = Actor.User("test@example.com");
        var command = new ChangeProductStatusCommand(productId, ProductStatus.InStock, actor);
        ...
        Assert.AreEqual(ProductStatus.InStock, productEntity.Status);
    ...
    public async Task Handle_ShouldPublishCorrectDomainEvent()
    {
        ...
        _mockMediator.Setup(x => x.Publish(It.IsAny<UpsertedProductDomainEvent>(), It.IsAny<CancellationToken>()))
            .Callback<object, CancellationToken>((e, ct) => publishedEvent = e as UpsertedProductDomainEvent)
            .Returns(Task.CompletedTask);
        ...
        Assert.AreEqual(ProductStatus.InStock, publishedEvent.Status);
    }
}
